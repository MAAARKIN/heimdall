package br.com.conductor.heimdall.gateway.azure;
...
import br.com.conductor.heimdall.core.environment.Property.Trace.Azure;
...
public class AzureLogger {
	...
	public AzureLogger(RestTemplateBuilder restTemplateBuilder, Property property) {
	...
	public void pushLogsToAzure(String json) {
		Azure azure = property.getTrace().getAzure();
		if (azure == null || StringUtils.isEmpty(azure.getWorkspaceId()) || StringUtils.isEmpty(azure.getSharedKey())) {
			return;
		}
	...
	private RequestEntity<String> constructHttpEntity(String json, Boolean isSysLog) {
		...
		String azueLogUrl = String.format(AZURE_LA_URL, property.getTrace().getAzure().getWorkspaceId());
	...
	private String createAuthorizationHeader(String canonicalizedString) {
		...
		try {
			...
			mac.init(new SecretKeySpec(Base64.getDecoder().decode(property.getTrace().getAzure().getSharedKey()), "HmacSHA256"));
			String authKey = new String(Base64.getEncoder().encode(mac.doFinal(canonicalizedString.getBytes("UTF-8"))));
			authStr = "SharedKey " + property.getTrace().getAzure().getWorkspaceId() + ":" + authKey;
		} catch (Exception e) {
